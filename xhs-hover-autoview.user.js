// ==UserScript==
// @name         小红书悬停自动打开关闭 & 自动翻页
// @namespace    http://tampermonkey.net/
// @version      0.0.1
// @description  悬停笔记自动打开，自动左右翻页，移出自动关闭
// @author       Qiwei
...
(function () {
    ...
    function setupConfig(name, key, minValue = 0) {
        GM_registerMenuCommand(`设置${name}`, () => {
    ...
    // 注册所有可配置项
    setupConfig("悬停自动打开延迟（毫秒）", "DELAY_OPEN_MS");
    setupConfig("自动关闭延迟（毫秒）", "DELAY_CLOSE_MS");
    setupConfig("滚动后鼠标移动触发阈值（像素）", "MOUSE_MOVE_THRESHOLD", 1);
    setupConfig("图片翻页边缘触发距离（像素）", "EDGE_THRESHOLD", 10);
    setupConfig("自动翻页点击间隔（毫秒）", "CLICK_INTERVAL_MS", 100);
...
//                     console.log('[XHS自动关闭] 鼠标移出，点击关闭');
        ...
        noteContainer.onmouseenter = () => {
            clearTimeout(closeTimeout);
            closeTimeout = null;
//             console.log('[XHS自动关闭] 鼠标移入，取消关闭');
        };

//         console.log('[XHS自动关闭] 已绑定关闭事件');
    }
    ...
    function handleCloseBlock() {
        ...
        onMouseMoveAfterBlock = (event) => {
            ...
            if (dist >= getVal('MOUSE_MOVE_THRESHOLD') && duration >= MIN_BLOCK_DURATION) {
                isBlocked = false;
//                 console.log('[XHS自动关闭后] 鼠标移动超过阈值且已过最小阻塞时间，解除阻塞');
                window.removeEventListener('mousemove', onMouseMoveAfterBlock);
                tryOpenNoteUnderMouse();
            }
        };
...
//                     console.log('[XHS自动翻页] 鼠标在左侧，自动点击左翻页');
                }
            } else if (autoDirection === 'right') {
                if (clickArrowIfAvailable('.arrow-controller.right .btn-wrapper')) {
//                     console.log('[XHS自动翻页] 鼠标在右侧，自动点击右翻页');
                }
            }
        }, getVal('CLICK_INTERVAL_MS'));
    })();

    console.log('[油猴-XHS自动点击] 已加载，当前参数：', {
        DELAY_OPEN_MS: getVal('DELAY_OPEN_MS'),
        DELAY_CLOSE_MS: getVal('DELAY_CLOSE_MS'),
        MOUSE_MOVE_THRESHOLD: getVal('MOUSE_MOVE_THRESHOLD'),
        EDGE_THRESHOLD: getVal('EDGE_THRESHOLD'),
        CLICK_INTERVAL_MS: getVal('CLICK_INTERVAL_MS'),
    });
})();
